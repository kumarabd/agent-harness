...
# loop worker, deploy/helm/agent-harness-tenant for the tenant worker and
# gateway — split per docs/components/multi-tenancy.md) rather than
# duplicated here, so `helm install` always picks up exactly what this built
# without a values.yaml edit, and they can't silently drift apart. All three
# Dockerfiles require the repo root as their build context — see the comment
# at the top of each for why (they COPY from workflows/ and/or activities/).

...
| Multi-organization / multi-tenant | N/A — not Hermes' design point either | Namespace-per-tenant; shared loop-worker pool + dedicated tenant workers (revised 2026-08-14 from dedicated-everything, via a reference-passing content contract); one self-hosted Postgres instance + PV per tenant | ✅ | `components/multi-tenancy.md`. |

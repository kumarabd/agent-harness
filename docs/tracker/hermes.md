...
| Multi-organization / multi-tenant | N/A — not Hermes' design point either | Namespace-per-tenant; shared loop-worker pool + dedicated tenant workers (revised 2026-08-14 from dedicated-everything, via a reference-passing content contract); one self-hosted Postgres instance per tenant (own dedicated PVC via the Bitnami subchart) plus a separate tenant PV for the session filesystem | ✅ | `components/multi-tenancy.md`. |

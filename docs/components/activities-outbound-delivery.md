...
**What this is not:** the primary mechanism protecting tenant content from a shared workflow-worker pool. Under the reference-passing contract, activities never hand the workflow raw content on the success path — the dominant path — so a panic has structurally little left to leak even if unhandled. Panic suppression closes the one residual seam (a serialization failure surfacing content-bearing *input* in a Temporal-generated failure, if inputs aren't reference-only) rather than being what makes sharing the workflow-worker pool safe in the first place. Recorded here explicitly so this isn't mistaken for the load-bearing isolation mechanism it was floated as before the reference-passing design existed.

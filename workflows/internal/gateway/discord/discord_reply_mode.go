...
func (b *Bot) handleModeCommand(ctx context.Context, dg *discordgo.Session, ic *discordgo.InteractionCreate) {
	opts := ic.ApplicationCommandData().Options
	if len(opts) == 0 {
		b.respondInteraction(dg, ic, "Usage: `/mode reply:voice` or `/mode reply:text`")
		return
	}
	mode := opts[0].StringValue()
	if mode != replyModeText && mode != replyModeVoice {
		b.respondInteraction(dg, ic, "Reply mode must be `voice` or `text`.")
		return
	}
	...
	if _, err := b.pool.Exec(ctx,
		...
		b.respondInteraction(dg, ic, "Couldn't save that — try again in a moment.")
		return
	}

	if mode == replyModeVoice {
		b.respondInteraction(dg, ic, "Reply mode set to **voice** — I'll answer with voice messages here. Anything you send, text or audio, is understood the same way.")
	} else {
		b.respondInteraction(dg, ic, "Reply mode set to **text** for this channel.")
	}
...
func (b *Bot) respondInteraction(dg *discordgo.Session, ic *discordgo.InteractionCreate, content string) {
...
func (b *Bot) sendPlainDiscordReply(dg *discordgo.Session, channelID, content string) {

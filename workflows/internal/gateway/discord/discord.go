...
// Run runs the Discord goroutine for this tenant's Gateway
...
func (b *Bot) Run(ctx context.Context) {
	// A fresh holder id per process start — the lease is about which process
	// currently holds a given live connection, not recognizing a process
	// across restarts.
	...
	dg.AddHandler(func(session *discordgo.Session, m *discordgo.MessageCreate) {
		b.discordMessageCreate(session, m, connectionID)
	})
	dg.AddHandler(func(session *discordgo.Session, ic *discordgo.InteractionCreate) {
		...
		case discordgo.InteractionApplicationCommand:
			...
			if ic.ApplicationCommandData().Name == "mode" {
				b.handleModeCommand(ctx, session, ic)
			} else {
				b.voice.HandleInteraction(ctx, session, ic, connectionID)
			}
		case discordgo.InteractionMessageComponent:
			b.discordUserInputInteractionCreate(ctx, session, ic, connectionID)
		}
	})
	...
	// only happens after dg.Open() (called later, inside runConnection).
	// One place registers every slash command: the discordvoice package's
	// /join and /leave plus this package's own /mode.
	registerCommands(dg, connectionID)

	for {
		ok, err := b.leases.AcquireOrRenew(ctx, "discord", connectionID, holderID, discordConnectionLeaseTTL)
		if err != nil {
			log.Printf("discord: lease acquire error: %v", err)
		}
		if ok {
			b.runConnection(ctx, dg, connectionID, holderID)
			// runConnection blocks until the connection drops or the
...
// runConnection holds one connection cycle: opens the live socket,
...
func (b *Bot) runConnection(ctx context.Context, dg *discordgo.Session, connectionID, holderID string) {
	if err := dg.Open(); err != nil {
		log.Printf("discord: failed to open connection: %v", err)
		return
	}
	defer dg.Close()
	defer b.leases.Release(context.Background(), "discord", connectionID, holderID)
...
// specific bot's own resolved identity (discord.Bot.Run), closed over
...
func (b *Bot) discordMessageCreate(session *discordgo.Session, m *discordgo.MessageCreate, connectionID string) {
	...
	if m.Flags&discordgo.MessageFlagsIsVoiceMessage != 0 {
		...
		if err != nil {
			...
			if isTrigger {
				b.sendPlainDiscordReply(session, m.ChannelID, "Sorry — I couldn't make out that voice message. Mind trying again or typing it?")
			}
			return
		}
		...
		if transcript == "" {
			...
			if isTrigger {
				b.sendPlainDiscordReply(session, m.ChannelID, "I didn't catch anything in that voice message — try again?")
			}
			return
		}
		if speech.IsFillerOnly(transcript) {
	...
	parentSessionKey := ""
	if replyTo != nil {
		rootID, err := b.resolveDiscordThreadRoot(ctx, m.ChannelID, *replyTo)
		if err != nil {
			log.Printf("discord: failed to resolve reply chain root: %v", err)
		} else if rootID != "" {
			discriminator = "reply_to_platform_message_id:" + rootID
			parentSessionKey = core.SessionKeyFor("discord", m.ChannelID, "channel:"+m.ChannelID)
		}
	}
	...
	if _, err := b.ingestor.Ingest(ctx, event); err != nil {
...
func (b *Bot) resolveDiscordThreadRoot(ctx context.Context, channelID, startMessageID string) (string, error) {

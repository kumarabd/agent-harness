...
// happened.
func (b *Bot) discordUserInputInteractionCreate(
	ctx context.Context,
	...
	// the exact prefix DeliverInterim's discordui.BuildUserInputComponents writes.
	...
	// splitting on the LAST colon instead: option_id (discordui.BuildUserInputComponents'
	...
	// a real round-trip test against discordui.BuildUserInputComponents' own output
	...
	if idx <= 0 || idx == len(rest)-1 {
		b.respondInteractionError(dg, ic.Interaction, "Malformed button — this shouldn't happen.")
		return
	}
	...
	if err != nil {
		b.respondInteractionError(dg, ic.Interaction, "That request no longer exists.")
		return
	}
	if channelID != ic.ChannelID {
		...
		// leaking which one.
		b.respondInteractionError(dg, ic.Interaction, "This button can't be used from this channel.")
		return
	}
	if status != "pending" {
		...
		b.respondInteractionUpdate(dg, ic, fmt.Sprintf("(this request was already %s)", status))
		return
	}
	...
	if err := b.temporal.SignalWorkflow(ctx, workflowID, "", wf.UserInputResponseSignalName, payload); err != nil {
		log.Printf("discord: user_input signal for %s failed: %v", requestID, err)
		b.respondInteractionError(dg, ic.Interaction, "Couldn't record that response — try again.")
		return
	}
...
func (b *Bot) respondInteractionError(dg *discordgo.Session, i *discordgo.Interaction, msg string) {
...
func (b *Bot) respondInteractionUpdate(dg *discordgo.Session, ic *discordgo.InteractionCreate, addendum string) {
	newContent := strings.TrimSpace(ic.Message.Content) + "\n\n" + addendum
	empty := []discordgo.MessageComponent{}
	if err := dg.InteractionRespond(ic.Interaction, &discordgo.InteractionResponse{
		Type: discordgo.InteractionResponseUpdateMessage,

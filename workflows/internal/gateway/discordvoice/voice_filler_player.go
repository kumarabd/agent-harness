...
// speech.SynthesizePCM returns, ready to feed straight into streamPCMToOpus
...
// entry via the same speech.SynthesizePCM path real turns use. Best-effort,
...
func synthesizeVoiceFillerCache(ctx context.Context) *voiceFillerCache {
	...
	for i, tier := range voiceProgressPhrases {
		stream, err := speech.SynthesizePCM(ctx, tier.Text)
		if err != nil {

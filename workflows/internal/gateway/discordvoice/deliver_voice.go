...
func (a *voiceDeliverActivity) Deliver(ctx context.Context, turnID string) error {
	...
	content = speech.SanitizeForSpeech(content)
	if content == "" {
	...
	ttsStart := time.Now()
	stream, err := speech.SynthesizePCM(ctx, content)
	if err != nil {
		return err
	}
	defer stream.Close()
...
func (a *voiceDeliverActivity) DeliverInterim(ctx context.Context, requestID string) error {
	...
	spoken = speech.SanitizeForSpeech(spoken)
	if spoken == "" {
	...
	stream, err := speech.SynthesizePCM(ctx, spoken)
	if err != nil {
		return err
	}
	defer stream.Close()
	...
	if len(options) > 0 {
		send := &discordgo.MessageSend{
			Content:    prompt,
			Components: discordui.BuildUserInputComponents(requestID, options),
		}
...
// speech.SynthesizePCM before this is called, never in how the resulting

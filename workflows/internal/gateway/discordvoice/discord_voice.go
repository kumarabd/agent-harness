...
// Commands — docs/components/gateway/discord-voice.md's "Resolved: Join
// Trigger": /join and /leave, not auto-join/auto-leave. Returned to the
// discord package, which registers every slash command (voice's plus its
// own /mode) in one place and routes interactions.
...
// HandleInteraction dispatches /join and /leave. Returns true if it handled
// the interaction, false if the command isn't one of ours (so the discord
// package can try its own handlers). shutdownCtx is the process's own
// shutdown-tied context — passed into voiceJoin as the parent for that
// connection's long-running goroutines, so a voice connection's lifetime is
// tied to the process's, not to context.Background().
func (v *Voice) HandleInteraction(shutdownCtx context.Context, dg *discordgo.Session, ic *discordgo.InteractionCreate, connectionID string) bool {
	...
	switch name {
	case "join":
		content = v.voiceJoin(shutdownCtx, dg, ic, connectionID)
	case "leave":
		content = v.voiceLeave(shutdownCtx, ic, connectionID)
	}
...
func (v *Voice) voiceJoin(ctx context.Context, dg *discordgo.Session, ic *discordgo.InteractionCreate, botConnectionID string) string {
	if ic.GuildID == "" {
	...
	ok, err := v.leases.AcquireOrRenew(ctx, "discord-voice", connectionID, holderID, voiceConnectionLeaseTTL)
	if err != nil {
	...
	if err != nil {
		_ = v.leases.Release(ctx, "discord-voice", connectionID, holderID)
	...
	filler := newVoiceFillerPlayer(v.fillerCache)
	deliverActivity := &voiceDeliverActivity{
		vc: vc, pool: v.pool, connectionID: connectionID,
	...
	v.state.mu.Lock()
	existing, hadExisting := v.state.byKey[voiceKey]
	v.state.byKey[voiceKey] = &activeVoiceConnection{vc: vc, connectionID: connectionID, holderID: holderID, cancel: cancel, deliverWkr: deliverWkr}
	v.state.mu.Unlock()
	...
	if hadExisting {
		...
		// the expiry window.
		_ = v.leases.Release(ctx, "discord-voice", existing.connectionID, existing.holderID)
	}

	go v.voiceLeaseRenewalLoop(connCtx, connectionID, holderID)
	go v.voiceCaptureLoop(connCtx, vc, channelID, connectionID, voiceKey, bargeIn, lifecycle, latency, filler)

	return "Joined."
}

func (v *Voice) voiceLeave(ctx context.Context, ic *discordgo.InteractionCreate, botConnectionID string) string {
	if ic.GuildID == "" {
		return "Voice channels don't exist in DMs."
	}
	voiceKey := ic.GuildID + ":" + botConnectionID
	if !v.teardownVoiceConnection(ctx, voiceKey, "user requested /leave") {
...
// down.
func (v *Voice) teardownVoiceConnection(ctx context.Context, voiceKey, reason string) bool {
	v.state.mu.Lock()
	entry, ok := v.state.byKey[voiceKey]
	if ok {
		delete(v.state.byKey, voiceKey)
	}
	v.state.mu.Unlock()
	if !ok {
		return false
	...
	_ = v.leases.Release(ctx, "discord-voice", entry.connectionID, entry.holderID)
	log.Printf("discord-voice: connection %s torn down (%s)", entry.connectionID, reason)
	return true
}

// voiceLeaseRenewalLoop mirrors discord.go's runConnection renewal
// loop, scoped to one voice connection instead of the whole bot's text
// connection.
func (v *Voice) voiceLeaseRenewalLoop(ctx context.Context, connectionID, holderID string) {
...
type speakerBuffer struct {
	...
	// in which case utterances go entirely through the batch speech.Transcribe
...
func (v *Voice) voiceCaptureLoop(ctx context.Context, vc *discordgo.VoiceConnection, channelID, connectionID, voiceKey string, bargeIn *voiceBargeIn, lifecycle *voiceLifecycle, latency *voiceLatencyTracker, filler *voiceFillerPlayer) {
	...
	// not continuously.
	var sttFactory func() *speech.WhisperLiveSession
	if url := speech.WhisperLiveURL(); url != "" {
		sttFactory = func() *speech.WhisperLiveSession { return speech.NewWhisperLiveSession(url) }
	}
	...
	flush := func(ssrc uint32, buf *speakerBuffer) {
		...
		go func() {
			...
			// request. Batch speech.Transcribe (voice_stt_tts.go) is the
			...
			if text == "" {
				...
				batchText, err := speech.Transcribe(context.Background(), wav)
				if err != nil {
			...
			if startedDuringPlayback && isBackchannelOnly(text) {
				...
				// back completely empty. Unlike speech.IsFillerOnly above, this
				// check is context-gated: the exact same transcript ("yeah")
				// said when the bot was NOT playing is a real one-word
				// answer, not a backchannel, and must reach core.Ingest
			...
			// injection — fired here, not after core.Ingest below:
			...
			if _, err := v.ingestor.Ingest(context.Background(), event); err != nil {
	...
	for {
		...
		case pkt, more := <-vc.OpusRecv:
			...
			if err := buf.vad.Err(); err != nil {
				...
				v.teardownVoiceConnection(context.Background(), voiceKey, "VAD sidecar failure")
				return
			}
			switch {

...
// speech.Transcribe path (voice_stt_tts.go), same as before this existed.
...
type WhisperLiveSession struct {
	...
	err             error
	// closed is set by close() — a permanent teardown (the voice connection
	// is going away), distinct from the transient server DISCONNECT that
	// readLoop transparently reconnects through. Once set, connect() is a
	// no-op and readLoop won't reconnect, so the WebSocket + its readLoop
	// goroutine don't leak past a /leave.
	closed bool
}
...
// close permanently tears the session down — called when the voice
// connection itself is going away (teardownVoiceConnection). Marks it
// closed (so a concurrent readLoop won't reconnect through it) and closes
// the WebSocket, which unblocks readLoop's ReadMessage and lets that
// goroutine exit. Idempotent. Distinct from the transient server-initiated
// DISCONNECT, which readLoop still reconnects through.
func (s *WhisperLiveSession) Close() {
	s.mu.Lock()
	if s.closed {
		s.mu.Unlock()
		return

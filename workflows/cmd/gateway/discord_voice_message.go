...
// already established (transcribeAudioFile → {SPEECH_BASE_URL}'s
...
func discordVoiceMessageContent(ctx context.Context, m *discordgo.MessageCreate) (string, error) {
	...
	if filename == "" {
		...
		// transcribeAudioFile needs to pick the right decode path.
		filename = "voice-message.ogg"
	}

	text, err := transcribeAudioFile(ctx, audioBytes, filename)
	if err != nil {

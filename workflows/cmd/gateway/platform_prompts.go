...
// DEFAULT_SYSTEM_PROMPT — that default is itself an "autonomous coding
// assistant, use shell_exec" framing (a scaffolding-era placeholder, per
// llm.py), which doesn't fit a spoken conversation at all regardless of
// formatting; patching it rather than replacing it wouldn't have fixed the
// actual complaint. Tool availability itself is unaffected either way —
// TOOLS_SCHEMA is passed to every ModelCall regardless of which system
// prompt is active, so this only changes framing/register, never what the

...
skill_ids="$(pg_query "SELECT string_agg(metadata->>'procedure_id', ',') FROM turn_retrieval WHERE episode_id = '$ROOT_TURN_ID' AND kind = 'skill'")"
...
# --- step 6: ComposeSkill ---
composed="$(pg_query "SELECT length(content) FROM turn_retrieval WHERE episode_id = '$ROOT_TURN_ID' AND kind = 'composed'")"
...
pg_query "SELECT metadata FROM turn_retrieval WHERE episode_id = '$ROOT_TURN_ID' AND kind = 'composed'" | grep -q "procedure_ids" \
...
plan_n="$(pg_query "SELECT count(*) FROM turn_plan WHERE episode_id = '$ROOT_TURN_ID' AND cp_id LIKE 'cp%'")"
...
cp1="$(pg_query "SELECT status FROM turn_plan WHERE episode_id = '$ROOT_TURN_ID' AND cp_id = 'cp1'")"
[ "$cp1" = "done" ] || fail "cp1 = '${cp1:-<missing>}', expected 'done' (scripted plan_progress step 1)"
cp2="$(pg_query "SELECT status FROM turn_plan WHERE episode_id = '$ROOT_TURN_ID' AND cp_id = 'cp2'")"
...
cp3="$(pg_query "SELECT status || '|' || coalesce(note,'') FROM turn_plan WHERE episode_id = '$ROOT_TURN_ID' AND cp_id = 'cp3'")"
...
cand=""
# ~55s: idleTTL (30s) + CloseSessionEpisodesWorkflow + RecordSkillOutcome +
# margin. skill_candidates.turn_id holds the episode_id (== root turn id here).

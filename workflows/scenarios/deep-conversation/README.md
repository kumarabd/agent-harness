...
| 3 | `../subagent-spawn.json` | scripted | Recursive child Turn Workflow + merge-back into parent context. Scripted here for determinism/cost, not necessity — **as of 2026-08-29, subagent spawning IS reachable by a real model** (`spawn_subagent` is a real `TOOLS_SCHEMA` entry, `is_subagent` is derived from the tool name in both providers, no longer hardcoded `false`). See `../README.md` and `../spawn-subagent-nested-valid.json`/`../spawn-subagent-nested-rejected.json` for the new recursion-termination-guard coverage this enabled. |

...
# Taxonomy & Per-Task Resolution"): discover_tools handler runs for real
...
#      the turn (discover_tools' own results still reach the model either way;
...
[ "$status" = "completed" ] || fail "root turn status = '$status', expected 'completed' — discover_tools' persist-on-discovery path may have crashed"
...
[ "${n_rows:-0}" -ge 1 ] || fail "no turn_retrieval kind='tool' rows for this turn — discover_tools found nothing to persist, or _persist_discovered didn't write (check mcp-hub/shell-hub have SOMETHING registered)"
ok "discover_tools persisted $n_rows discovered tool row(s) into turn_retrieval"
